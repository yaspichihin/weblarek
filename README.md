...
- `clear()` - очищает форму (вызывает `reset()`)
...
**Обработка:** Событие генерируется при выборе товара, но напрямую не обрабатывается (используется через геттер getSelectedProduct() при необходимости)
...
**Данные:** нет  
**Обработка:** Обработчик определяет какая форма открыта (order или contacts) и вызывает соответствующую валидацию (validateOrder или validateContacts)
...
**Данные:** `{ id: string }`  
**Обработка:** Открытие модального окна с CardPreview выбранного товара
...
**Обработка:** Сохранение в модель Buyer, запуск валидации формы
...
**Источник:** Presenter (функция validateOrder)  
**Когда:** Изменилось состояние валидности формы оплаты  
**Данные:** `{ valid: boolean, errors: string[] }`  
**Обработка:** Обновление состояния формы (блокировка кнопки, отображение ошибок)
...
**Данные:** `{ email: string }`  
**Обработка:** Сохранение в модель Buyer, запуск валидации
...
**Источник:** Presenter (функция validateContacts)  
**Когда:** Изменилось состояние валидности формы контактов  
**Данные:** `{ valid: boolean, errors: string[] }`  
**Обработка:** Обновление состояния формы (блокировка кнопки, отображение ошибок)
...
**Данные:** нет  
**Обработка:** Отправка заказа на сервер, отображение Success экрана
...
**Данные:** нет  
**Обработка:** Очистка корзины и данных покупателя, закрытие модального окна, перезагрузка каталога
...
→ buyer.setPayment/setAddress → buyer:changed
→ Валидация → order:valid
→ Активация кнопки "Далее"

order:submit
→ Открытие ContactsForm

contacts:email + contacts:phone
→ buyer.setEmail/setPhone → buyer:changed
→ Валидация → contacts:valid
...
success:close
→ cart.clearCart() → cart:changed → header обновляется
→ buyer.clearBuyerInfo() → buyer:changed
→ modal:close
```
...
1. Пользователь изменяет поле → событие `{form}:{field}`
2. Presenter обновляет модель → `buyer.set{Field}()` → `buyer:changed`
3. Presenter запускает валидацию → `buyer.validate()`
4. Presenter генерирует → `{form}:valid` с результатами
5. View обновляет форму → блокирует кнопку / показывает ошибки
...
- `validateOrder()` - валидация формы оплаты (проверка способа оплаты и адреса)
- `validateContacts()` - валидация формы контактов (проверка email и телефона)

**Основной поток:**

1. `init()` - загрузка данных с API
2. Обработка событий от моделей и представлений
3. Координация взаимодействия между компонентами
4. Валидация форм в реальном времени
5. Отправка заказа на сервер
...
- Темплейты клонируются для каждого нового компонента
...
3. **Элементы находятся в конструкторе** - все DOM элементы ищутся один раз при создании и сохраняются в поля
4. **Слушатели в конструкторе** - все обработчики событий устанавливаются один раз при создании
5. **Метод render()** - всегда возвращает HTMLElement (корневой элемент компонента)
